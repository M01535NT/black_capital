        ...
        <div className="space-y-6 max-w-4xl mx-auto w-full">
            ...
            <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-display-3 font-semibold text-foreground">{lead.full_name}</h2>
                        <p className="text-foreground/50 text-body-sm">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* Status + Source */}
                    <div className="bg-card border border-foreground/10 rounded-2xl p-6 space-y-4">
                        <h3 className="text-caption text-foreground/50">Detalles del Lead</h3>
                        <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-1">
                                <p className="text-caption text-foreground/50">Estado Actual</p>
                                <Badge variant="secondary" className={(statusMap[lead.status] || {}).color || ""}>
                                    {(statusMap[lead.status] || {}).label || lead.status}
                                </Badge>
                            </div>
                            <div className="space-y-1">
                                <p className="text-caption text-foreground/50">Origen</p>
                            ...
                            <div className="space-y-1">
                                <p className="text-caption text-foreground/50">Fecha de Registro</p>
                                <div className="flex items-center gap-2 text-body-sm">
                            ...
                            <div className="space-y-1">
                                <p className="text-caption text-foreground/50">ID</p>
                                    <div className="flex items-center gap-2 text-body-sm font-sans tabular-nums text-foreground/60">
                    ...
                    {/* Notes */}
                    <div className="bg-card border border-foreground/10 rounded-2xl p-6 space-y-4">
                        <h3 className="text-caption text-foreground/50">Notas Internas</h3>
                        {lead.notes ? (
                                <div className="text-body text-foreground/70 whitespace-pre-wrap leading-relaxed bg-muted/20 p-4 rounded-xl border border-foreground/5">
                                {lead.notes}
                            </div>
                        ) : (
                            <p className="text-body-sm text-foreground/50 italic">Sin notas registradas.</p>
                        )}
                    </div>
                    ...
                    <div className="bg-card border border-foreground/10 rounded-2xl p-6 space-y-4">
                        ...
                        {activities && activities.length > 0 ? (
                            ...
                            <p className="text-body-sm text-foreground/50 italic">Sin actividad registrada.</p>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-card border border-foreground/10 rounded-2xl p-5 space-y-3">
                        <h3 className="text-caption text-foreground/50">Acciones Rápidas</h3>
                        <a href={`mailto:${lead.email}`}
                            className="flex items-center justify-center gap-2 w-full py-2.5 px-4 border border-foreground/20 rounded-xl text-body-sm font-medium hover:bg-muted/50 transition-colors"
                        >
                    ...
                    <div className="bg-card border border-foreground/10 rounded-2xl p-5 space-y-3">
                        <h3 className="text-caption text-foreground/50">Cambiar Estado</h3>
