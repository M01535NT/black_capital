...
function NavGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2.5">
      <h3 className="footer-heading-type text-[var(--color-gold-light)]/90">
        {title}
      </h3>
      {children}
    </div>
...
export function MobileDrawer({ pathname }: MobileDrawerProps) {
  ...
  return (
    <Drawer direction="right" open={open} onOpenChange={setOpen}>
      <DrawerTrigger asChild>
        <Button
          ...
          className={cn(
            ...
            "border-white/12 bg-black/20 text-foreground hover:border-[var(--color-gold-light)]/55 hover:bg-white/[0.06]",
            "focus-visible:ring-2 focus-visible:ring-[var(--color-gold-light)] focus-visible:ring-offset-2 focus-visible:ring-offset-background",
            open && "border-[var(--color-gold-light)]/55 bg-[var(--color-gold-light)]/10",
          )}
          aria-label="Abrir navegación"
        >
          <span className="absolute inset-0 bg-[radial-gradient(circle_at_center,var(--color-gold-light)_0%,transparent_62%)] opacity-0 transition-opacity duration-300 group-hover:opacity-10" />
      ...
      <DrawerContent className="h-screen top-0 right-0 left-auto mt-0 w-80 sm:w-96 lg:w-[420px] rounded-none bg-[#0a0a0a]/95 backdrop-blur-2xl border-l border-[var(--color-gold-light)]/15">
        <div className="mx-auto flex h-full w-full max-w-sm flex-col overflow-hidden p-4 sm:p-5">
          <DrawerHeader className="shrink-0 px-0 pb-2 pt-0 text-left flex flex-row items-center justify-end">
            <button
              onClick={handleClose}
              className="text-foreground/60 hover:text-[var(--color-gold-light)] transition-colors p-1"
              ...
              <div className="grid grid-cols-1 gap-[clamp(1rem,3vw,1.75rem)]">
                {visibleBaseLinks.map((link) => (
                    <Link
                      ...
                      isActive(link.href) ? "border-[var(--color-gold-light)]/45 text-[var(--color-gold-light)]" : "text-foreground hover:border-[var(--color-gold-light)]/40 hover:text-[var(--color-gold-light)]",
                    )}
                    onClick={handleClose}
                ...
                <div className="grid gap-[clamp(1rem,3vw,1.75rem)]">
                  ...
                  <Link key={link.name} href={link.href} onClick={handleClose} className="group border border-white/[0.06] bg-white/[0.02] px-3 py-2 focus-visible:outline-none">
                      <span className={cn(
                        ...
                        isActive(link.href) ? "text-[var(--color-gold-light)]" : "text-foreground group-hover:text-[var(--color-gold-light)]",
                      )}>
                        {link.name}
                      </span>
                  ...
                  {herramientasDropdown.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        onClick={handleClose}
                      >
                        <Icon className="w-3.5 h-3.5 text-[var(--color-gold-light)]/80" aria-hidden="true" />
                        {item.name}
                      </Link>
                    );
                  })}
                ...
                <div className="grid grid-cols-1 gap-2.5">
                  {corporativoLinks.map((link) => (
                    <Link
                      ...
                      className={cn(
                        "font-display hero-mobile-secondary-link transition-colors",
                        isActive(link.href) ? "text-[var(--color-gold-light)]" : "text-foreground/80 hover:text-[var(--color-gold-light)]",
                      )}
                      onClick={handleClose}
                    >
                      {link.name}
